...
from seo.config import get_config, server_option
from seo.database import db
from seo.middlewares import sentry_middleware_factory, request_context_middleware
from seo.routes import setup_routes
...
async def on_app_startup(app: web.Application):
    ...
    # Starting up...
    app.logger.info("Starting up seo-worker on address: {host}:{port}...".format(host=host, port=port))
    ...
    # Started!
    app.logger.info("seo-worker successfully started on {host}:{port}!".format(host=host, port=port))
...
async def on_app_cleanup(app: web.Application):
    ...
    """
    # Cleaning up...
    app.logger.info("Cleaning up seo-worker's resources...")

    # Clean up the database resources
    db.cleanup()

    # Cleaned up!
    app.logger.info("seo-worker's resources were successfully cleaned up!")
...
async def on_app_shutdown(app: web.Application):
    ...
    :param app: The application instance the signal is being executed for.
    """
    app.logger.info("Shutting down seo-worker...")
...
async def create_app() -> web.Application:
    ...
    _app = web.Application(
        logger=_logger,
        middlewares=[
            # sentry_middleware_factory(),  # TODO need for sentry
    ...
    api_v1_0_swagger_doc_path = str(PurePath(this_modules_path, "seo/docs/swagger-v1.0.yaml"))
