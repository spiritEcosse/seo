...
SHELL_NAME = bash
RUN_DEV_SERVER = adev runserver -v seo/app.py --app-factory create_app -p 8137
COMPOSE_FILE_AB?=../../docker-compose.yml
MESSAGE?=
...
dev-upgrade-deps: clean install
	...
	@. $(VENV_DIR)/bin/activate \
		...
		&& pip freeze | egrep -v seo > requirements.txt
	@echo "===================================================="
...
cover coverage:
	...
	@test -d $(TEST_VENV_DIR) || { echo "Please run 'make install-dev' first!"; exit 1; }
	@. $(TEST_VENV_DIR)/bin/activate \
		&& coverage run --source seo/ `which py.test` --cache-clear -$(VERBOSITY) tests/unit_tests; \
		coverage report; \
		coverage html;
...
#################################
up-deps:
	docker-compose -f ${COMPOSE_FILE_AB} up -d postgres
...
shell:
	docker-compose run --rm -it seo ipython

bash:
	docker-compose run --rm seo bash

makemigrations:
	docker-compose run --rm seo bash -c "alembic revision --autogenerate -m '${MESSAGE}'"

migrate:
	docker-compose run --rm seo bash -c "alembic upgrade head"

build-up: build up-deps up
